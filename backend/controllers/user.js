...
exports.getProfile = async (req, res) => {
  try {
    ...
    const posts = await Post.find({ user: profile._id })
      ...
      .populate(
        "comments.commentBy",
        "first_nmae last_name picture username commentAt"
      )
      .sort({ createdAt: -1 });
...
exports.addFriend = async (req, res) => {
  try {
    if (req.user.id !== req.params.id) {
      ...
      return res
        .status(400)
        .json({ message: "You can not send a request to yourself." });
    }
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
exports.cancelRequest = async (req, res) => {
  try {
    if (req.user.id !== req.params.id) {
      ...
      return res
        .status(400)
        .json({ message: "You can not send or cancel a request to yourself." });
    }
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
exports.acceptRequest = async (req, res) => {
  try {
    if (req.user.id !== req.params.id) {
      ...
      return res
        .status(400)
        .json({ message: "You can not accept a request from yourself." });
    }
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
  ...
  try {
    if (req.user.id !== req.params.id) {
      ...
      if (
        ...
        return res.status(400).json({ message: "Unfriended already." });
      }
    } else {
      return res
        .status(400)
  ...
  try {
    if (req.user.id !== req.params.id) {
      ...
      if (receiver.requests.includes(sender._id)) {
        ...
        return res.status(400).json({ message: "Deleted already." });
      }
    } else {
      return res
        .status(400)
        .json({ message: "You can not send or delete a request yourself." });
    }
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
